...
standard_deviation<-function(x=numeric(),y){
  variance<-0
  y<-mean(x)
  for (i in (1:length(x))){
    variance<-(variance+((x[i]-y)*(x[i]-y)))
  }
  deviation<-sqrt(variance/length(x))
  print(deviation)
  return(deviation)
}``

standard_error1<-function(x,y){
  error<-standard_deviation(x,y)/(sqrt(length(x)))
  return(error)
}
x<-c(17.2, 22.1, 18.5, 17.2, 18.6, 14.8, 21.7, 15.8, 16.3, 22.8,  24.1, 13.3, 
